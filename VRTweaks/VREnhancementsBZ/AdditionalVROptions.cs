        ...
        class GeneralTab_VROptionsPatch
        {
            static void Postfix(uGUI_OptionsPanel __instance)
            {
              /*  __instance.AddHeading(generalTabIndex, "General VR Options");
                __instance.AddToggleOption(generalTabIndex, "Enable VR Animations", VRGameOptions.enableVrAnimations, delegate (bool v)
                {
                    VRGameOptions.enableVrAnimations = v;
                ...
                }SliderLabelMode.Percent,"00");
                __instance.AddHeading(generalTabIndex, "VR User Interface Options");
                __instance.AddSliderOption(generalTabIndex, "Subtitle Height", subtitleHeight, 20, 75, 25,1, delegate (float v)
                {
                    subtitleHeight = v;
                    UIElementsFixes.SetSubtitleHeight(subtitleHeight);
                },SliderLabelMode.Float,"00");
                __instance.AddSliderOption(generalTabIndex, "Subtitle Scale", subtitleScale * 100, 50, 150, 100,1, delegate (float v)
                {
                    subtitleScale = v / 100;
                    UIElementsFixes.SetSubtitleScale(subtitleScale);
                },SliderLabelMode.Float,"000");
                /*__instance.AddSliderOption(generalTabIndex, "PDA Distance", PDA_Distance * 100f, 25, 40, 40,1, delegate (float v)
                {
                    PDA_Distance = v / 100f;
                ...
                __instance.AddSliderOption(generalTabIndex, "HUD Opacity", HUD_Alpha * 100f, 40, 100, 70,0.1f, delegate (float v)
                {
                    HUD_Alpha = v / 100f;
                    UIElementsFixes.UpdateHUDOpacity(HUD_Alpha);
                }, SliderLabelMode.Float, "000");
                __instance.AddSliderOption(generalTabIndex, "HUD Distance", HUD_Distance / 0.5f, 2, 4, 3,0.1f, delegate (float v)
                {
                    HUD_Distance = v * 0.5f;
                    UIElementsFixes.UpdateHUDDistance(HUD_Distance);
                }, SliderLabelMode.Float, "000");
                __instance.AddSliderOption(generalTabIndex, "HUD Scale", HUD_Scale / 0.5f, 1, 4, 2,0.1f, delegate (float v)
                {
                    HUD_Scale = v * 0.5f;
